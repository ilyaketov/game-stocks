...
def write_report(period_start, period_end, flat, source_bytes, output_path=None):
    ...
    def calc_risk_pct(store):
        ...
        # Знаменатель — полный остаток магазина (все строки), как в листе «Свод»,
        # чтобы «Остаток магазина, $» совпадал между листами.
        total = sum(r['end_val'] for r in flat if r['store'] == store)
    ...
    for i, store in enumerate(stores_sorted_by_risk):
        ...
        ws3.cell(row=r, column=10, value=f'=SUMIF({T_STORE},$A{r},{T_END_VAL})')
        ws3.cell(row=r, column=11, value=f'=IFERROR(MIN(1,I{r}/J{r}),0)')
