...
HEADERS = [
    "Подразделение", "Игра", "Артикул",
    "Остаток шт. на начало", "Продано, шт.", "Закуплено, шт.", "Перемещения, шт.", "Остаток шт. на конец",
    "Остаток $ на начало", "Продано, $", "Закуплено, $", "Перемещения, $", "Остаток $ на конец",
    "Дней с последнего закупа", "Движение",
]
WIDTHS = [22, 50, 12, 13, 12, 13, 13, 13, 14, 13, 14, 13, 14, 14, 11]
QTY_COLS = (4, 5, 6, 7, 8)        # начало, продано, закуплено, перемещения, конец
MONEY_COLS = (9, 10, 11, 12, 13)  # ... аналогично в долларах
DAYS_COL = 14
MOVEMENT_COL = 15
...
def _apply_data_format(ws, row_idx, col_idx, font=None):
    ...
    if col_idx in (1, 2, 3):
        ...
        c.alignment = RIGHT
    elif col_idx == DAYS_COL:
        c.number_format = '0;;-'
        c.alignment = CENTER
    elif col_idx == MOVEMENT_COL:
        c.alignment = CENTER
...
def write_report(period_start, period_end, flat, source_bytes, output_path=None):
    ...
    for r in flat_sorted_t1:
        ...
        ws1.append([
            r['store'], r['game'], r['sku'],
            r['start_qty'], r['sold_qty'], r['bought_qty'], r['net_move_qty'], r['end_qty'],
            r['start_val'], r['sold_val'], r['bought_val'], r['net_move_val'], r['end_val'],
    ...
    for r in flat_sorted_t2:
        ...
        ws2.cell(row=row_cursor, column=4, value=r['start_qty'])
        ws2.cell(row=row_cursor, column=5, value=r['sold_qty'])
        ws2.cell(row=row_cursor, column=6, value=r['bought_qty'])
        ws2.cell(row=row_cursor, column=7, value=r['net_move_qty'])
        ...
        ws2.cell(row=row_cursor, column=12, value=r['net_move_val'])
        ws2.cell(row=row_cursor, column=13, value=r['end_val'])
        ws2.cell(row=row_cursor, column=14, value=r['days_since_bill'])
        ws2.cell(row=row_cursor, column=15, value=movement_text)
        for col_idx in range(1, len(HEADERS) + 1):
    ...
    ws_svod.merge_cells(start_row=1, start_column=1, end_row=1, end_column=11)
    ws_svod['A2'] = ""

    pivot_headers = [
        "Подразделение", "Остаток шт. на начало", "Продано, шт.", "Закуплено, шт.", "Перемещения, шт.", "Остаток шт. на конец",
        "Остаток $ на начало", "Продано, $", "Закуплено, $", "Перемещения, $", "Остаток $ на конец",
    ]
    phr = 3
    ...
    # Колонки Таблицы (15): D=start_qty, E=sold_qty, F=bought_qty, G=net_move_qty, H=end_qty,
    #                       I=start_val, J=sold_val, K=bought_val, L=net_move_val, M=end_val,
    #                       N=days, O=movement
    # Свод показывает: start_qty, sold_qty, bought_qty, net_move_qty, end_qty,
    #                  start_val, sold_val, bought_val, net_move_val, end_val
    # чтобы тождество нач − продано + закуплено + перемещения = конец закрывалось.
    src_cols_map = {2: 'D', 3: 'E', 4: 'F', 5: 'G', 6: 'H', 7: 'I', 8: 'J', 9: 'K', 10: 'L', 11: 'M'}
    qty_cols = (2, 3, 4, 5, 6)
    start_data = phr + 1
    ...
    pivot_widths = [24, 15, 13, 14, 15, 15, 17, 14, 15, 15, 17]
    ...
    T = "Таблица"
    # Новые колонки (15): G=net_move_qty, H=end_qty, L=net_move_val, M=end_val, N=days, O=movement
    T_DAYS = f"{T}!$N${T1_FIRST}:$N${T1_LAST}"
    T_END_QTY = f"{T}!$H${T1_FIRST}:$H${T1_LAST}"
    T_END_VAL = f"{T}!$M${T1_FIRST}:$M${T1_LAST}"
    T_STORE = f"{T}!$A${T1_FIRST}:$A${T1_LAST}"

    # Склады-накопители (магазины без единой закупки за всю историю) исключаются
    # из анализа рисков: их сток приходит только внутренними перемещениями, поэтому
    # «дней с последнего закупа» и «мёртвый сток» для них не имеют смысла.
    warehouse_stores = sorted({r['store'] for r in flat if r.get('is_warehouse')})
    _wh_set = set(warehouse_stores)
    risk_stores = [s for s in stores if s not in _wh_set]
    # Суффикс условий для COUNTIFS/SUMIFS, исключающий склады из категорийных итогов.
    wh_excl = ''.join(f',{T_STORE},"<>{wh}"' for wh in warehouse_stores)
    ...
    ws3['A2'] = (f"Игры в стоке: цвет — по дням с последнего закупа; "
                 ...
                 + (f"  •  Склады-накопители исключены: {', '.join(warehouse_stores)}"
                    if warehouse_stores else ""))
    ...
    for i, (label, desc, fill, lo_op, hi_op) in enumerate(cat_data):
        ...
        if hi_op:
            ws3.cell(row=r, column=3, value=f'=COUNTIFS({T_END_QTY},">0",{T_DAYS},"{lo_op}",{T_DAYS},"{hi_op}"{wh_excl})')
            ws3.cell(row=r, column=4, value=f'=SUMIFS({T_END_QTY},{T_END_QTY},">0",{T_DAYS},"{lo_op}",{T_DAYS},"{hi_op}"{wh_excl})')
            ws3.cell(row=r, column=5, value=f'=SUMIFS({T_END_VAL},{T_END_QTY},">0",{T_DAYS},"{lo_op}",{T_DAYS},"{hi_op}"{wh_excl})')
        else:
            ws3.cell(row=r, column=3, value=f'=COUNTIFS({T_END_QTY},">0",{T_DAYS},"{lo_op}"{wh_excl})')
            ws3.cell(row=r, column=4, value=f'=SUMIFS({T_END_QTY},{T_END_QTY},">0",{T_DAYS},"{lo_op}"{wh_excl})')
            ws3.cell(row=r, column=5, value=f'=SUMIFS({T_END_VAL},{T_END_QTY},">0",{T_DAYS},"{lo_op}"{wh_excl})')
    ...
    def calc_risk_pct(store):
        ...
        # Знаменатель — живой остаток (позиции с шт > 0). Это верная база для доли
        # риска: стареющее — подмножество живого стока, поэтому доля всегда ≤ 100%.
        # НЕ равен «Остатку $ на конец» из Свода: тот включает распроданные позиции
        # с отрицательным остаточным cost'ом (FIFO-хвосты). Это разные метрики.
        total = sum(r['end_val'] for r in flat if r['store'] == store and r['end_qty'] > 0)
        return (problem / total) if total > 0 else 0

    stores_sorted_by_risk = sorted(risk_stores, key=lambda s: -calc_risk_pct(s))
    ...
    if not top15:
        ...
        for i, r in enumerate(top15):
            ...
            ws3.cell(row=rr, column=2, value=f"=Таблица!N{src_r}" if src_r else r['days_since_bill'])
            ...
            ws3.cell(row=rr, column=7, value=f"=Таблица!M{src_r}" if src_r else r['end_val'])
    ...
    never_sold_count = sum(1 for r in flat if not r['has_any_sale_ever'] and r['end_qty'] > 0 and not r.get('is_warehouse'))
    no_movement_count = sum(1 for r in flat if not r['has_movement_in_period'] and r['end_qty'] > 0 and not r.get('is_warehouse'))
