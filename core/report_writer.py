...
WIDTHS = [22, 50, 12, 13, 13, 12, 13, 13, 13, 14, 13, 14, 13, 14, 14, 11]
QTY_COLS = (5, 6, 7, 8, 9)       # начало, продано, закуплено, перемещения, конец
MONEY_COLS = (10, 11, 12, 13, 14)  # ... аналогично в долларах
DAYS_COL = 15
MOVEMENT_COL = 16
...
def write_report(period_start, period_end, flat, source_bytes, output_path=None):
    ...
    for r in flat_sorted_t2:
        ...
        ws2.cell(row=row_cursor, column=8, value=r['net_move_qty'])
        ws2.cell(row=row_cursor, column=9, value=r['end_qty'])
        ws2.cell(row=row_cursor, column=10, value=r['start_val'])
        ws2.cell(row=row_cursor, column=11, value=r['sold_val'])
        ws2.cell(row=row_cursor, column=12, value=r['bought_val'])
        ws2.cell(row=row_cursor, column=13, value=r['net_move_val'])
        ws2.cell(row=row_cursor, column=14, value=r['end_val'])
        ws2.cell(row=row_cursor, column=15, value=r['days_since_bill'])
        ws2.cell(row=row_cursor, column=16, value=movement_text)
        for col_idx in range(1, len(HEADERS) + 1):
    ...
    # Колонки Таблицы: E=start_qty, F=sold_qty, G=bought_qty, H=net_move_qty, I=end_qty,
    #                  J=start_val, K=sold_val, L=bought_val, M=net_move_val, N=end_val, O=days, P=movement
    # Свод показывает: start_qty, sold_qty, bought_qty, end_qty, start_val, sold_val, bought_val, end_val
    src_cols_map = {2: 'E', 3: 'F', 4: 'G', 5: 'I', 6: 'J', 7: 'K', 8: 'L', 9: 'N'}
    start_data = phr + 1
    ...
    T = "Таблица"
    # Новые колонки: H=net_move_qty, I=end_qty, M=net_move_val, N=end_val, O=days, P=movement
    T_DAYS = f"{T}!$O${T1_FIRST}:$O${T1_LAST}"
    T_END_QTY = f"{T}!$I${T1_FIRST}:$I${T1_LAST}"
    T_END_VAL = f"{T}!$N${T1_FIRST}:$N${T1_LAST}"
    ...
    if not top15:
        ...
        for i, r in enumerate(top15):
            ...
            ws3.cell(row=rr, column=2, value=f"=Таблица!O{src_r}" if src_r else r['days_since_bill'])
            ...
            ws3.cell(row=rr, column=6, value=f"=Таблица!I{src_r}" if src_r else r['end_qty'])
            ws3.cell(row=rr, column=7, value=f"=Таблица!N{src_r}" if src_r else r['end_val'])
